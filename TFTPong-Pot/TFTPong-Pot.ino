...
#define sensor1Pin A14 // sensor one
...
void loop()
{
  ...
  //sensor1Val = analogRead(sensorPin);
  int sensor2Val = analogRead(sensor2Pin) - 512;
  //Serial.println(String(sensor2Val));
  sensor2Val = sensor2Val / 230;
  ...
  if (time > ball_update && gameIsRunning)
  {
    ...
    display.drawFastVLine(ball_x - 1, ball_y, 2, BLACK);
    display.drawFastVLine(ball_x, ball_y, 2, BLACK);

    display.drawFastVLine(new_x - 1, new_y, 2, WHITE);
...
void gameOver()
{
  ...
  if (PLAYER_SCORE > CPU_SCORE)
  {
    ...
    display.print("Mila wins!:P");
  } else
  {
    display.setCursor(5, 4);
    display.setTextColor(WHITE);
    display.setTextSize(2);
    display.print("You were vanquished:(HAHA!");
  }

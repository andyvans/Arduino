...
void loop(void) {
  ...
  sensor1Val = analogRead(sensorPin)/8; // read sensor data
  sensor2Val = analogRead(sensor2Pin)/8; // read sensor data
  ...
  else // if there is no touch event do this
  {
    ...
    if (pauseState == 0) // start / stop drawing data on touch
    {
      ...
      int sensor2Transposed = sensor2Val;//(sensor2Val - tft.height()) / 2 ;
